...
class TestPipelineNotifyClass(TestRunner):
    def test_email(self):
        pipeline = Pipeline(notify=[NotifyEmail(email="dev@acmeinc.com")])
        self.validator.check_result(
            pipeline, {"steps": [], "notify": [{"email": "dev@acmeinc.com"}]}
        )
    ...
    def test_slack(self):
        pipeline = Pipeline(notify=[NotifySlack(slack="#channel")])
        self.validator.check_result(
            pipeline, {"steps": [], "notify": [{"slack": "#channel"}]}
        )

    def test_slack_if(self):
        pipeline = Pipeline(
            notify=[NotifySlack(slack="#channel", step_if="build.state == 'failed")]
        )
        self.validator.check_result(
            pipeline,
            {
                "steps": [],
                "notify": [{"slack": "#channel", "if": "build.state == 'failed"}],
            },
        )

    def test_slack_channels_list(self):
        pipeline = Pipeline(
            notify=[NotifySlack(slack=NotifySlackObject(channels=["#one", "#two"]))]
        )
        self.validator.check_result(
            pipeline,
            {"steps": [], "notify": [{"slack": {"channels": ["#one", "#two"]}}]},
        )
    ...
    def test_slack_message(self):
        pipeline = Pipeline(
            notify=[
                NotifySlack(
                    slack=NotifySlackObject(
                        channels=["#one", "#two"], message="a message"
                    )
                )
            ]
        )
    ...
    def test_pagerduty(self):
        ...
        self.validator.check_result(
            ...
            {
                "steps": [],
                "notify": [{"pagerduty_change_event": "636d22Yourc0418Key3b49eee3e8"}],
            },
        )
    ...
    def test_github_commit_status_string(self):
        ...
        self.validator.check_result(
            pipeline, {"steps": [], "notify": ["github_commit_status"]}
        )
    ...
    def test_github_commit_status(self):
        ...
        self.validator.check_result(
            ...
            {
                "steps": [],
                "notify": [{"github_commit_status": {"context": "my-custom-status"}}],
            },
        )
...
class TestPipelineNotifyDict(TestRunner):
    def test_email(self):
        pipeline = Pipeline(
            notify=[NotifyEmail.from_dict({"email": "dev@acmeinc.com"})]
        )
        self.validator.check_result(
            pipeline, {"steps": [], "notify": [{"email": "dev@acmeinc.com"}]}
        )
    ...
    def test_slack(self):
        pipeline = Pipeline(notify=[NotifySlack.from_dict({"slack": "#channel"})])
        self.validator.check_result(
            pipeline, {"steps": [], "notify": [{"slack": "#channel"}]}
        )
    ...
    def test_slack_channels_list(self):
        pipeline = Pipeline(
            notify=[NotifySlack.from_dict({"slack": {"channels": ["#one", "#two"]}})]
        )
        self.validator.check_result(
            pipeline,
            {"steps": [], "notify": [{"slack": {"channels": ["#one", "#two"]}}]},
    ...
    def test_github_commit_status(self):
        pipeline = Pipeline(
            notify=[
                NotifyGithubCommitStatus.from_dict(
                    {"github_commit_status": {"context": "my-custom-status"}}
                )
            ]
        )
...
class TestCommandNotifyClass(TestRunner):
    ...
    def test_slack(self):
        ...
        pipeline = Pipeline(
            steps=[
                CommandStep(command="bash.sh", notify=[NotifySlack(slack="#channel")])
            ]
        )
        self.validator.check_result(pipeline, {"steps": [expected]})
    ...
    def test_github_check(self):
        ...
        pipeline = Pipeline(
            steps=[CommandStep(command="bash.sh", notify=["github_check"])]
        )
        self.validator.check_result(pipeline, {"steps": [expected]})
    ...
    def test_github_commit_status_string(self):
        ...
        pipeline = Pipeline(
            steps=[CommandStep(command="bash.sh", notify=["github_commit_status"])]
        )
        self.validator.check_result(pipeline, {"steps": [expected]})
